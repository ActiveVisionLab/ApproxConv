    ...
    def __init__(
        ...
        if order is not None:
            self.dwconv = operation(
                dim, dim, kernel_size=7, padding=3, groups=dim, order=order
            )  # depthwise conv
        else:
            self.dwconv = operation(
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        if orders is not None and orders[0] != -1:
            stem = nn.Sequential(
                operation(in_chans, dims[0], kernel_size=4, stride=4, order=orders[0]),
            ...
            stage = nn.Sequential(
                *[
                    Block(
                        ...
                        operation=operation,
                        order=orders[1 + i][j] if orders is not None else None,
                    )
